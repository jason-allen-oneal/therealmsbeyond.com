...
export const getPointFromTouch = (touch: React.PointerEvent) => {
  return {
    x: touch.clientX,
    y: touch.clientY,
  };
...
class GalleryBox extends React.Component<Props, State> {
  width: number;
  height: number;
  onNavigationCallback: (index: number) => void;
  animation: number | undefined;
  lastTouchEnd: boolean | number | undefined;
  swipeStartX!: number;
  swipeStartY!: number;
  lastDistance!: number;
  ...
  handleTouchEnd(event: React.TouchEvent) {
    ...
    if (
      this.lastTouchEnd &&
      (this.lastTouchEnd as number) + DOUBLE_TAP_THRESHOLD > event.timeStamp
    ) {
      this.reset();
    }
  ...
  handleSwipe(event: React.TouchEvent) {
